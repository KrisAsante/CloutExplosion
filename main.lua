...
local badCharacter2 = display.newImage( "./assets/sprites/enemy2.png" )
badCharacter2.x = 1620
badCharacter2.y = display.contentHeight - 1000
badCharacter2.id = "bad character"
physics.addBody( badCharacter2, "dynamic", { 
    friction = 0.5, 
    bounce = 0.3 
    } )

local Baku = display.newImage( "./assets/sprites/character.png" )
Baku.x = display.contentCenterX - 200
Baku.y = display.contentCenterY
Baku.id = "the character"
physics.addBody( Baku, "dynamic", { 
    density = 3.0, 
    friction = 0.5, 
    bounce = 0.3 
    } )
Baku.isFixedRotation = true -- If you apply this property before the physics.addBody() command for the object, it will merely be treated as a property of the object like any other custom property and, in that case, it will not cause any physical change in terms of locking rotation.
...
local function checkCharacterPosition( event )
    ...
    if Baku.y > display.contentHeight + 500 then
        Baku.x = display.contentCenterX - 200
        Baku.y = display.contentCenterY
    end
end
...
local function onCollision( event )
    ...
    if ( event.phase == "began" ) then
        ...
        if ( ( obj1.id == "bad character" and obj2.id == "bullet" ) or
			...
			local emitterParams = {
			    startColorAlpha = 1,
			    startParticleSizeVariance = 2000,
...
function shootButton:touch( event )
    if ( event.phase == "began" ) then
        ...
        local aSingleBullet = display.newImage( "./assets/sprites/Clout.png" )
        aSingleBullet.x = Baku.x
        aSingleBullet.y = Baku.y
